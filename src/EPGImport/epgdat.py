...
class epgdat_class:
	...
	def preprocess_events_channel(self, services):
		...
		for service in services:
			...
			for event in events:
				...
				# if not epg_event_description_dict.has_key(shortDescription[0]):
				# if not exist_event(shortDescription[0]) :
				if not self.EPGDAT_HASH_EVENT_MEMORY_CONTAINER.has_key(shortDescription[0]):
					# DESCRIPTION DATA
					pack_1 = s_BB.pack(0x4d, len(shortDescription[1])) + shortDescription[1]
					...
					# save_event(shortDescription[0], [pack_1,1])
					self.EPGDAT_HASH_EVENT_MEMORY_CONTAINER[shortDescription[0]] = [pack_1, 1]
				...
				longDescription = event[3]
				EPG_EVENT_HEADER_datasize += 4 * len(longDescription)  # add 4 bytes for a single REF DESC (CRC32)
				for desc in longDescription:
				...
				pack_4 = s_I.pack(shortDescription[0])  # REF DESC short (title)
				for d in longDescription:
	...
	def final_process(self):
		if self.EPG_TOTAL_EVENTS > 0:
			...
			# HEADER 1
			pack_1 = pack(self.LB_ENDIAN + "I13sI", 0x98765432, 'ENIGMA_EPG_V7', self.EPG_HEADER1_channel_count)
			...
			for temp in sorted(self.EPGDAT_HASH_EVENT_MEMORY_CONTAINER.keys()):
				...
				# pack_1 = pack(LB_ENDIAN+"II", int(temp,16), pack_2[1])
